...
export async function POST(req: Request, ctx: { params: Promise<{ id: string; action: string }> }) {
  ...
  switch (action) {
    case "approve":
      return done(approveIssue(id, u, { projectId: body.projectId as string | undefined }));
    case "reject":
      return done(rejectIssue(id, u, String(body.reason ?? "")));
    case "undo":
      return done(undoIssue(id, u));
    case "remind":
      return remind(id, u.name) ? json({ ok: true, message: "Reminder sent" }) : json({ error: "There's no open question to remind about" }, 409);
    case "answer":
      if (!issue.question_id) return json({ error: "No open question" }, 409);
      return done(answerQuestion(issue.question_id, { value: String(body.value ?? ""), label: body.label as string | undefined }, { name: u.name, via: "answered in IES" }));
    case "send-to-expert":
      return done(await sendToExpert(id, String(body.expertId ?? ""), String(body.question ?? "").trim(), u));
    case "install":
      return done(installAgent(String(body.agentId ?? ""), (body.entities as string[]) ?? [], u, id));
    case "reply":
      ...
      return done(replyToExpert(issue.case_id, u, String(body.body).trim()));
    default:
