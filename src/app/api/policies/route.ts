...
export const POST = handler(async (req) => {
  ...
  for (const p of body.policies ?? []) {
    ...
    await run("update policies set mode=?, limit_amount=? where key=?", mode, limit, p.key);
  }
  if (body.materiality !== undefined && Number(body.materiality) >= 0) await setSetting("materiality", String(Math.round(Number(body.materiality))));
