...
export async function demandBoard(): Promise<Demand[]> {
  const hpWaiting = !!(await one("select id from issues where kind='labor' and status='needs_agent'"));
  const published = (await one<{ n: number }>("select count(*)::int n from agents where status='published'"))?.n ?? 0;
  return [
...
export async function devState(developerId: string): Promise<DevState> {
  const a = await one<AgentRow>("select * from agents where developer_id=? order by created_at desc limit 1", developerId);
  const agent = a
    ? (() => {
  ...
  return {
    agent,
    demand: await demandBoard(),
    catalog: PERMISSION_CATALOG,
    usage: a ? await all<DevState["usage"][number]>("select id, company, detail, amount, dev_share, created_at from agent_usage where agent_id=? order by id desc", a.id) : [],
    installs: a ? await all<DevState["installs"][number]>("select id, entities, installed_at, revoked_at from installs where agent_id=? order by id desc", a.id) : [],
  };
}

export async function createAgent(developerId: string) {
  const existing = await one<{ id: string }>("select id from agents where developer_id=?", developerId);
...
export async function saveConfig(developerId: string, patch: Partial<AgentConfig> & { name?: string; summary?: string; price?: number }) {
  const a = await one<AgentRow>("select * from agents where developer_id=?", developerId);
...
export async function evaluate(developerId: string) {
  const a = await one<AgentRow>("select * from agents where developer_id=?", developerId);
  if (!a) return { ok: false, message: "Create the agent first" };
  const cfg = JSON.parse(a.config) as AgentConfig;
  const res = runEvaluation(cfg);
  await run("update agents set eval=?, eval_config=?, status=? where id=?", JSON.stringify({ ...res, ranAt: nowIso() }), configKey(cfg), a.status === "published" ? "published" : res.allPass ? "evaluated" : "draft", a.id);
  return { ok: true, message: res.allPass ? `All ${res.total} cases passed` : `${res.passed} of ${res.total} passed`, allPass: res.allPass };
}

export async function submitSecurity(developerId: string) {
  const a = await one<AgentRow>("select * from agents where developer_id=?", developerId);
  if (!a) return { ok: false, message: "Create the agent first" };
  const cfg = JSON.parse(a.config) as AgentConfig;
  ...
  await run("update agents set security=? where id=?", JSON.stringify({ status: "approved", at: nowIso(), note }), a.id);
  return { ok: true, message: "Security and permissions review approved" };
}

export async function publish(developerId: string) {
  const a = await one<AgentRow>("select * from agents where developer_id=?", developerId);
  if (!a) return { ok: false, message: "Create the agent first" };
  const cfg = JSON.parse(a.config) as AgentConfig;
  const ev = a.eval ? (JSON.parse(a.eval) as { allPass: boolean }) : null;
  ...
  await run("update agents set status='published', published_at=? where id=?", nowIso(), a.id);
