...
export function lookups() {
  const users = new Map(all<UserRow>("select id,name,role,title,phone,firm,initials,color,email from users").map((u) => [u.id, u]));
  const projects = new Map(all<Project>("select * from projects").map((p) => [p.id, p]));
  const accounts = new Map(all<{ id: string; number: string; name: string; type: string }>("select * from accounts").map((a) => [a.id, a]));
  const vendors = new Map(all<{ id: string; name: string }>("select * from vendors").map((v) => [v.id, v.name]));
  return { users, projects, accounts, vendors };
}

export function policies(): Policy[] {
  return all<Policy>("select * from policies order by sort");
}
export function policy(key: string): Policy {
  return one<Policy>("select * from policies where key=?", key)!;
}
export function materiality(): number {
  return Number(getSetting("materiality", "5000"));
}
...
export function projectMargins(): Margins {
  const L = lookups();
  const sums = all<{ project_id: string | null; account_id: string; s: number }>(
...
export function publishedAgent(): AgentCard | null {
  const a = one<AgentRow>("select * from agents where status='published' order by published_at desc limit 1");
  if (!a) return null;
...
export function activeInstall(agentId: string) {
...
function questionView(qid: string | null) {
  if (!qid) return null;
  const q = one<QuestionRow>("select * from questions where id=?", qid);
  if (!q) return null;
  const u = lookups().users.get(q.asked_user_id);
...
function caseView(caseId: string | null) {
  if (!caseId) return null;
  const c = one<CaseRow>("select * from cases where id=?", caseId);
  if (!c) return null;
  const expert = lookups().users.get(c.expert_id);
  const messages = all<{ id: number; from_name: string; to_name: string; kind: string; body: string; created_at: string }>("select * from case_messages where case_id=? order by id", caseId);
...
export function issueViews(runId: string) {
  const L = lookups();
  const mat = materiality();
  const rec = publishedAgent();
  const issues = all<IssueRow>("select * from issues where run_id=? order by created_at, rowid", runId);
  return issues.map((i) => {
    const txn = i.txn_id ? one<Txn>("select * from transactions where id=?", i.txn_id) : undefined;
    const receipt = i.txn_id ? one<{ file: string }>("select file from receipts where matched_txn_id=?", i.txn_id) : undefined;
    const activity = all<ActivityRow>("select * from activity where issue_id=? order by id", i.id);
    const install = rec ? activeInstall(rec.id) : undefined;
    ...
    return {
      ...
      timesheets: movedWorkers.length
        ? all<{ worker: string; project_id: string; week_start: string; hours: number; approved_by: string }>(`select worker, project_id, week_start, hours, approved_by from time_entries where worker in (${movedWorkers.map(() => "?").join(",")}) order by worker, week_start`, ...movedWorkers)
        : [],
      ...
      question: questionView(i.question_id),
      case: caseView(i.case_id),
      activity,
      recommendation: i.kind === "labor" && rec ? { ...rec, installed: !!install, installEntities: install ? (JSON.parse(install.entities) as string[]) : [] } : null,
    };
  });
}
export type IssueView = ReturnType<typeof issueViews>[number];

// ---------- Readiness ----------
export function readiness() {
  const runRow = one<{ id: string }>("select id from runs order by started_at desc limit 1");
  const mat = materiality();
  ...
  const crewTime = getSetting("sitelog_time_at", "");
  const sources = [
    { name: "Bank feeds", at: getSetting("bank_synced_at"), state: "fresh" },
    { name: "Payroll PR-0930", at: getSetting("payroll_posted_at"), state: "fresh" },
    { name: "SiteLog daily logs", at: getSetting("sitelog_logs_at"), state: "fresh" },
    { name: "SiteLog crew time", at: crewTime || null, state: crewTime ? "fresh" : "not_connected" },
  ];
  const applied = one<{ n: number }>("select count(*) n from activity where kind='applied'")?.n ?? 0;
  const awaiting = open.filter((i) => ["needs_approval", "expert_returned", "needs_expert", "needs_agent"].includes(i.status)).length;
  const expertSigned = one<{ n: number }>("select count(*) n from cases where status in ('returned','approved')")?.n ?? 0;
  const approvedByYou = one<{ n: number }>("select count(*) n from activity where kind='approved' and actor_type='person'")?.n ?? 0;
  return {
    reviewed: !!runRow,
    ready: !!runRow && blockers.length === 0,
    reviewDate: getSetting("review_date", "2026-10-02"),
    materiality: mat,
    blockers,
